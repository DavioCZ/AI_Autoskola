# scraper_okruh4.py  –  vytěží celý okruh 4
# -*- coding: utf-8 -*-
...
OUT    = "okruh4.json"  # Změna pro okruh 4
...
# ---------------------------------------------------------------------------
def get_question_links(okruh=4): # Změna pro okruh 4
...
def parse_question(url: str) -> dict | None:
    ...
    # 3) Obrázek NEBO video otázky
    q_media = None                  # můžeme ponechat stejný klíč, nebo založit 'video'
    # a) nejdřív zkus obrázek
    img_tag = soup.select_one("div.image-frame img")
    if img_tag and img_tag.get("src"):
        img_src = img_tag["src"]
        if img_src and not img_src.isspace():
            url_img = urljoin(url, img_src)
            if img_exists(url_img): # img_exists ověřuje existenci URL, ne typ souboru
                q_media = url_img
            else:
                print(f"⚠️  Obrázek otázky {qid} nenalezen na URL: {url_img} (zdroj: {img_src})")
        else:
            print(f"⚠️  Prázdný src atribut pro obrázek otázky {qid}")

    # b) když se obrázek nenašel, hledej video
    if q_media is None:
        # <video src="...">  ▸ jednoduchý případ
        vid = soup.select_one("div.image-frame video[src]")
        if vid and vid.get("src"):
            video_src = vid["src"]
            if video_src and not video_src.isspace():
                q_media = urljoin(url, video_src)
                # Zde bychom mohli přidat ověření existence videa, pokud je to nutné,
                # např. upravenou funkcí img_exists nebo novou video_exists
        else:
            # někdy je src až v <source>
            src_tag = soup.select_one("div.image-frame video source[src]")
            if src_tag and src_tag.get("src"):
                source_src = src_tag["src"]
                if source_src and not source_src.isspace():
                    q_media = urljoin(url, source_src)
                    # Podobně zde můžeme ověřit existenci

    # Pokud q_media zůstane None, znamená to, že ani obrázek ani video nebylo nalezeno
    ...
    return {
        "id":       qid,
        "otazka":   q_text,
        "obrazek":  q_media, # Použijeme q_media místo q_img
        "moznosti": opts,
        "spravna":  correct,
    }
...
if __name__ == "__main__":
    links = get_question_links(4) # Změna pro okruh 4
    print(f"Okruh 4: {len(links)} odkazů nalezeno") # Změna pro okruh 4
    all_q  = []
